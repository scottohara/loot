...
export default class LootStatesProvider {
	...
	public constructor($stateProvider: angular.ui.IStateProvider) {
				...
				resolve: {
					...
					context: [
						...
						(
							...
							authenticated && contextModel.find(Number($stateParams.id)),
					],
					transactionBatch: [
						"authenticated",
						"transactionModel",
		...
		$stateProvider
			...
			.state(
				"root.accounts.account.transactions.transaction",
				transactionState(),
			)
			...
			.state(
				"root.categories.category.transactions",
				transactionsState("category"),
			)
			.state(
				"root.categories.category.transactions.transaction",
				transactionState(),
			)
			...
			.state(
				"root.securities.security.transactions",
				transactionsState("security"),
			)
			.state(
				"root.securities.security.transactions.transaction",
				transactionState(),
			)
