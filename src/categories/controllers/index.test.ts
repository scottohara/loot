...
describe("CategoryIndexController", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				...
				categoryIndexController = controllerTest(
					"CategoryIndexController",
				) as CategoryIndexController;
			},
		) as Mocha.HookFunction,
	);
	...
	it("should focus the category when a category id is specified", (): void => {
		...
		$timeout.flush();
		expect(
			(categoryIndexController.tableActions as OgTableActionHandlers).focusRow,
		).to.have.been.calledWith(0);
	});

	it("should not focus the category when a category id is not specified", (): void =>
	...
	it("should ensure the category is focussed when the category id state param changes", (): void => {
		...
		sinon.stub(
			categoryIndexController,
			"focusCategory" as keyof CategoryIndexController,
		);
	...
	describe("editCategory", (): void => {
		...
		describe("(edit existing)", (): void => {
			...
			it("should not attempt to decrement original parent's children count if there was no original parent", (): void => {
				...
				const originalCategories: Category[] = angular.copy(
			...
			it("should increment the new parent's children count when the parent category changes", (): void => {
				...
				$uibModal.close(category);
				expect(
					Number(categoryIndexController.categories[5].num_children),
				).to.equal(3);
			});
		...
		describe("(add new)", (): void => {
			...
			it("should open the edit category modal without a category", (): void => {
				...
				expect(($uibModal.resolves as UibModalMockResolves).category).to.be
					.undefined;
			});
		...
		it("should resort the categories list when the modal is closed", (): void => {
			const outflowCategoryWithHighestName: Category = angular.copy(
			...
			expect(
				categoryIndexController.categories.pop() as Category,
			).to.deep.equal(outflowCategoryWithHighestName);
		});
		...
		it("should show an alert if the category has transactions", (): void => {
			...
			expect(
				(($uibModal.resolves as UibModalMockResolves).alert as OgModalAlert)
					.header,
			).to.equal("Category has existing transactions");
		});
	...
	describe("toggleFavourite", (): void => {
		...
		beforeEach(
			(): Category[] => ([category] = categoryIndexController.categories),
		);
		...
		afterEach(
			(): Chai.Assertion =>
				expect(categoryModel.toggleFavourite).to.have.been.called,
		);
	});
