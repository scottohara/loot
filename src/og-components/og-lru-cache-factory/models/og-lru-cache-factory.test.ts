...
describe("ogLruCacheFactory", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(_ogLruCacheFactory_: OgLruCacheFactory): OgLruCacheFactory =>
	...
	describe("new", (): void => {
		...
		it("should return an LruCache", (): Chai.Assertion =>
			expect(ogLruCacheFactory.new(10, [])).to.be.an("object"));

		describe("LruCache (empty)", (): void => {
			beforeEach(
				(): OgLruCache => (ogLruCache = ogLruCacheFactory.new(10, [])),
			);

			it("should have the specified capacity", (): Chai.Assertion =>
				expect(ogLruCache["capacity"]).to.equal(10));

			it("should have no items", (): Chai.Assertion =>
				expect(ogLruCache.list).to.be.an("array").that.is.empty);
		});

		describe("LruCache", (): void => {
			const capacity = 10;
			let data: OgCacheEntry[], list: OgCacheEntry[];

			beforeEach((): void => {
				// Populate the data with capacity + 1 items (to check that the oldest item is evicted)
				data = [...Array(capacity + 1).keys()]
					.reverse()
					.map((id: number): OgCacheEntry => ({ id, name: `item ${id}` }));
			...
			it("should be populated with the initial set of items", (): Chai.Assertion =>
				expect(ogLruCache["items"]).to.deep.equal(list));

			describe("put", (): void => {
				it("should leave the list unchanged if the item is already the current head", (): Chai.Assertion =>
					expect(
						ogLruCache.put({ id: capacity, name: `item ${capacity}` }),
					).to.deep.equal(list));
				...
				const scenarios: {
					...
					{
						description:
							"move an existing item from the tail of the list to the head of the list",
			...
			describe("remove", (): void => {
				it("should leave the list unchanged if the item does not exist", (): Chai.Assertion =>
					expect(ogLruCache.remove(-1)).to.deep.equal(list));
				...
				describe("existing item", (): void => {
					...
					it("should remove the only item from the list", (): void => {
						id = 1;
						ogLruCache["items"] = [{ id, name: `item ${id}` }];
					...
					it("should remove an item from the tail of the list", (): void => {
						id = 1;
						ogLruCache.remove(id);
						expect(ogLruCache["items"]).to.deep.equal(
							list.slice(0, capacity - 1),
						);
					});
			...
			describe("list", (): void => {
				it("should return the cached items in order", (): Chai.Assertion =>
					expect(ogLruCache.list).to.deep.equal(list));
			});
		});
	});
});
