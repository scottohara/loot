...
export default class OgTableNavigableDirective {
	constructor($window, ogTableNavigableService) {
		return {
			restrict: "A",
			scope: {
			...
			link(scope, iElement) {
				...
				scope.getRows = () => $window.$(iElement).children("tbody").children("tr");
				...
				scope.scrollToRow = row => {
					...
					const	rowTop = row.offset().top,
								...
								viewTop = $window.$(document).scrollTop(),
								viewBottom = viewTop + $window.$(window).height(),
	...
	static factory($window, ogTableNavigableService) {
