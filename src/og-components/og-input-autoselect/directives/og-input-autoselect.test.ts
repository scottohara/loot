...
describe("ogInputAutoselect", (): void => {
				...
				$window.$.withArgs(
					sinon.match(
						(value: JQuery<Element>): boolean =>
							value[0] === ogInputAutoselect["element"][0],
					),
	...
	describe("isFocussed", (): void => {
		...
		it("should be true if the passed element is the document's active element", (): Chai.Assertion =>
			expect(scope.isFocussed(document.activeElement as Element)).to.be.true);

		it("should be false if the element is not the document's active element", (): Chai.Assertion =>
			expect(scope.isFocussed()).to.be.false);
	});
