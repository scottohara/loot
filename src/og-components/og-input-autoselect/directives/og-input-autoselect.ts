...
export default class OgInputAutoSelectDirective {
	...
	public static factory($window: angular.IWindowService, $timeout: angular.ITimeoutService, ogModalErrorService: OgModalErrorService): OgInputAutoSelectDirective {
		return new OgInputAutoSelectDirective($window, $timeout, ogModalErrorService);
	}
}

OgInputAutoSelectDirective.factory.$inject = ["$window", "$timeout", "ogModalErrorService"];