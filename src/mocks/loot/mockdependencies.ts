...
export default class MockDependenciesProvider {
	public constructor(
		private readonly $provide: angular.auto.IProvideService,
			...
			(dependency: string): void => {
				...
				this.$provide.value(
					...
					this.$injector.invoke(
						mockProvider.$get.bind(mockProvider),
						mockProvider,
					),
				);
			},
...
describe("mockDependenciesProvider", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				_mockDependencies_: MockDependenciesProvider,
	...
	describe("$get", (): void => {
		it("should return the mockDependencies provider", (): Chai.Assertion =>
			expect(mockDependenciesProvider).to.have.property("load"));
	});
});
