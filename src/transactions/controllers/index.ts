...
export default class TransactionIndexController {
	...
	public readonly loading: { prev: boolean; next: boolean } = {
		prev: false,
		next: false,
	};
	...
	public constructor(
		...
		this.processTransactions(
			transactionBatch,
			undefined,
			Number(this.$state.params.transactionId),
		);
		...
		$scope.$on(
			...
			$transitions.onSuccess(
				...
				(transition: angular.ui.IState): void =>
					this.transitionSuccessHandler(
						Number(transition.params("to").transactionId),
					),
			) as () => void,
		);

		// Auto scroll to the bottom
		$timeout((): void => ogViewScrollService.scrollTo("bottom")).catch(
			this.showError,
		);
	}
	...
	public getTransactions(
		...
		if (undefined === getFromDate) {
			const fromIndex: number =
				"prev" === direction ? 0 : this.transactions.length - 1;

			// Get the from date (depending on which direction we're fetching)
			if (
				undefined !== (this.transactions[fromIndex] as Transaction | undefined)
			) {
		...
		if (undefined === this.contextType) {
			...
			transactionFetch = this.transactionModel.query(
			...
			transactionFetch = this.transactionModel.all(
				(this.contextModel as EntityModel).path((this.context as Entity).id),
				getFromDate as Date,
				direction,
				this.unreconciledOnly,
			);
		...
		transactionFetch
			.then((transactionBatch: TransactionBatch): void => {
	...
	public toggleUnreconciledOnly(
		...
		if (!this.reconciling) {
			...
			this.accountModel.unreconciledOnly(
				Number((this.context as Entity).id),
				unreconciledOnly,
			);
	...
	public save(): void {
		this.accountModel
			.reconcile(Number((this.context as Entity).id))
			.then((): void => {
				// Remove the closing balance from local storage
				this.$window.localStorage.removeItem(
					`lootClosingBalance-${(this.context as Entity).id}`,
				);
		...
		if (!this.reconciling) {
			...
			this.$uibModal
				...
				.result.then((closingBalance: number): void => {
					...
					this.accountModel
						.find(Number((this.context as Entity).id))
						.then((account: Account): void => {
							this.reconcileTarget = Number(
								(
									this.closingBalance - account.reconciled_closing_balance
								).toFixed(decimalPlaces),
							);
							this.clearedTotal = Number(
								account.cleared_closing_balance.toFixed(decimalPlaces),
							);
							this.unclearedTotal = Number(
								(this.reconcileTarget - this.clearedTotal).toFixed(
									decimalPlaces,
								),
							);
	...
	public switchAccount(
		$event: Event | null,
		transaction: TransferrableTransaction,
	): void {
		this.switchToAccount(
			$event,
			Number((transaction.account as Entity).id),
			transaction as Transaction,
		);
	}

	// Switch to the primary account of a transaction
	public switchPrimaryAccount(
		$event: Event | null,
		transaction: Transaction,
	): void {
		this.switchToAccount(
			$event,
			Number(transaction.primary_account.id),
			transaction,
		);
	}
	...
	public switchPayee($event: Event, transaction: PayeeCashTransaction): void {
		this.switchTo(
			$event,
			"payees.payee",
			Number((transaction.payee as Entity).id),
			transaction as Transaction,
		);
	}
	...
	public switchSecurity($event: Event, transaction: SecurityTransaction): void {
		this.switchTo(
			$event,
			"securities.security",
			Number((transaction.security as Entity).id),
			transaction as Transaction,
		);
	}
	...
	public switchCategory(
		$event: Event,
		transaction: CategorisableTransaction,
	): void {
		this.switchTo(
			$event,
			"categories.category",
			Number((transaction.category as Entity).id),
			transaction as Transaction,
		);
	}

	// Switch to the transaction's subcategory
	public switchSubcategory(
		$event: Event,
		transaction: SubcategorisableTransaction,
	): void {
		this.switchTo(
			$event,
			"categories.category",
			Number((transaction.subcategory as Entity).id),
			transaction as Transaction,
		);
	}
	...
	private editTransaction(index?: number): void {
		...
		if (
			!isNaN(Number(index)) &&
			!this.isAllowed("edit", this.transactions[Number(index)])
		) {
			return;
		}
				...
				resolve: {
					transaction: ():
							...
							switch (this.contextType) {
								...
								case "category":
									newTransaction.category =
										(this.context as Category).parent ??
										(this.context as Category);
									newTransaction.subcategory =
										undefined === (this.context as Category).parent ||
										null === (this.context as Category).parent
											? null
											: (this.context as Category);
									break;
						...
						switch (this.transactions[Number(index)].transaction_type) {
							...
							case "Payslip":
								(
									this.transactions[Number(index)] as SplitTransaction
								).subtransactions = [];

								return this.transactionModel
									.findSubtransactions(
										Number(this.transactions[Number(index)].id),
									)
									.then(
										(subtransactions: SplitTransactionChild[]): Transaction => {
											(
												this.transactions[Number(index)] as SplitTransaction
			...
			.result.then((transaction: Transaction): void => {
				...
				if (this.contextChanged(transaction)) {
					...
					this.updateClosingBalance(
						this.transactions[Number(index)],
						transaction,
					);
					...
					if (
						...
						this.getTransactions(
							"next",
							subDays(transaction.transaction_date as Date, 1),
							Number(transaction.id),
						);
					} else if (
						!isBefore(
	...
	private contextChanged(transaction: Transaction): boolean {
		...
		switch (this.contextType) {
			...
			case "security":
				currentContext = (transaction as SecurityTransaction)
			...
			case "category":
				currentContext = (
					...
					null === (this.context as Category).parent
						...
						: (transaction as SubcategorisableTransaction).subcategory
		...
		return undefined === currentContext || null === currentContext
			? false
			: currentContext.id !== (this.context as Entity).id;
	}
	...
	private deleteTransaction(index: number): void {
		...
		this.$uibModal
			...
			.result.then((): void => this.removeTransaction(index))
			.finally((): true => (this.ogTableNavigableService.enabled = true))
			.catch(this.showError);
	}
	...
	private updateClosingBalance(
		...
		if (
			"object" === typeof this.context &&
			Object.getOwnPropertyDescriptor(this.context, "closing_balance")
		) {
	...
	private isAllowed(
		...
		switch (transaction.transaction_type) {
			...
			case "SecurityInvestment":
				if (
					"investment" !==
						transaction.primary_account.account_type.toLowerCase() &&
					"edit" === action
				) {
					allowed = false;
	...
	private promptToSwitchAccounts(
		...
		this.$uibModal
			...
			.result.then((): void => {
				...
				if (
					null !== (transaction as TransferrableTransaction).account &&
					Number(
						((transaction as TransferrableTransaction).account as Account).id,
					)
				) {
	...
	private processTransactions(
		...
		if (transactionBatch.transactions.length) {
			...
			this.firstTransactionDate = transactionBatch.transactions[0]
				.transaction_date as Date;
			this.lastTransactionDate = transactionBatch.transactions[
	...
	private updateRunningBalances(): void {
		...
		if (
			undefined !==
				((this.context as Account).account_type as AccountType | undefined) &&
			"investment" === (this.context as Account).account_type.toLowerCase()
		) {
			return;
		}

		this.transactions.reduce(
			(openingBalance: number, transaction: Transaction): number => {
				transaction.balance =
					openingBalance +
					(transaction as CashTransaction).amount *
						("inflow" === transaction.direction ? 1 : -1);
	...
	private focusTransaction(transactionIdToFocus: number): number {
		...
		angular.forEach(
			this.transactions,
			(transaction: Transaction, index: number): void => {
	...
	private updateReconciledTotals(transaction: Transaction): void {
		...
		this.unclearedTotal = Number(
			(this.reconcileTarget - this.clearedTotal).toFixed(decimalPlaces),
		);
	}
	...
	private switchTo(
		...
		this.$state
			.go(`root.${state}.transactions.transaction`, {
				id,
...
TransactionIndexController.$inject = [
	"$scope",
	"$transitions",
	"$uibModal",
	"$timeout",
