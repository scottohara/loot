...
export default class TransactionModel {
	...
	public all(context: string, fromDate: Date | null, direction: TransactionFetchDirection, unreconciledOnly: boolean): angular.IPromise<TransactionBatch> {
	...
	public query(query: string, fromDate: Date | null, direction: TransactionFetchDirection): angular.IPromise<TransactionBatch> {
	...
	public findSubtransactions(id: number): angular.IPromise<SplitTransactionChild[]> {
		return this.$http.get(`${this.path(id)}/subtransactions`).then((response: angular.IHttpResponse<SplitTransactionChild[]>): SplitTransactionChild[] => response.data);
	}

	// Retrieves a single transaction
	public find(id: number): angular.IPromise<Transaction> {
		return this.$http.get(this.path(id)).then((response: angular.IHttpResponse<Transaction>): Transaction => this.parse(response.data));
	}
	...
	private invalidateCaches(transaction: Transaction): void {
		...
		this.invalidateCache(this.payeeModel, (transaction as PayeeCashTransaction).payee as Payee);
		this.invalidateCache(this.categoryModel, (transaction as CategorisableTransaction).category as Category);
		this.invalidateCache(this.categoryModel, (transaction as SubcategorisableTransaction).subcategory as Category);
		this.invalidateCache(this.accountModel, (transaction as TransferrableTransaction).account);
		this.invalidateCache(this.securityModel, (transaction as SecurityTransaction).security as Security);

		// Subtransactions
		angular.forEach((transaction as SplitTransaction).subtransactions, (subtransaction: SplitTransactionChild): void => {
			this.invalidateCache(this.categoryModel, subtransaction.category as Category);
			this.invalidateCache(this.categoryModel, (subtransaction as SubcategorisableTransaction).subcategory as Category);
			this.invalidateCache(this.accountModel, (subtransaction as TransferrableTransaction).account);
		});
	}

	// Helper function to handle a single $http cache invalidation
	private invalidateCache(itemModel: EntityModel, item: NewOrExistingEntity | null): void {
