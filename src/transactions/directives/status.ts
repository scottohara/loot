...
export default class TransactionStatusDirective {
					...
					directive: angular.IDirective = {
						...
						link(scope: TransactionStatusScope, iElement: JQuery<Element>): void {
							...
							scope.clickHandler = (): void => {
								...
								transactionModel.updateStatus(accountModel.path(scope.transactionStatus.account.id), Number(scope.transactionStatus.transaction.id), status)
	...
	public static factory($sce: angular.ISCEService, transactionModel: TransactionModel, accountModel: AccountModel, ogModalErrorService: OgModalErrorService): TransactionStatusDirective {
		return new TransactionStatusDirective($sce, transactionModel, accountModel, ogModalErrorService);
	}
}

TransactionStatusDirective.factory.$inject = ["$sce", "transactionModel", "accountModel", "ogModalErrorService"];