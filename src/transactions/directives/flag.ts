...
export default class TransactionFlagDirective {
	public constructor($sce: angular.ISCEService,
					...
					directive: angular.IDirective = {
						...
						link(scope: TransactionFlagScope, iElement: JQuery<Element>): void {
							function setTooltip(): void {
								scope.tooltip = $sce.trustAsHtml(scope.transaction.flag_type ? `<strong class="${scope.transaction.flag_type}">${FlagTypes[scope.transaction.flag_type]}</strong><br/>${scope.transaction.flag}` : "Flag transaction...") as string;
							}
	...
	public static factory($sce: angular.ISCEService,
						$uibModal: angular.ui.bootstrap.IModalService,
						ogTableNavigableService: OgTableNavigableService,
						ogModalErrorService: OgModalErrorService): TransactionFlagDirective {
		return new TransactionFlagDirective($sce, $uibModal, ogTableNavigableService, ogModalErrorService);
	}
}

TransactionFlagDirective.factory.$inject = ["$sce", "$uibModal", "ogTableNavigableService", "ogModalErrorService"];