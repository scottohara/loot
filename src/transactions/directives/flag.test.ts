...
describe("transactionFlag", (): void => {
	...
	const scenarios: { flagType: TransactionFlagType; tooltip: string }[] = [
		...
		},
		{
			...
			tooltip:
				'<strong class="taxdeductible">Tax Deductible</strong><br/>Test flag',
		},
	];
	...
	scenarios.forEach(
		(scenario: { flagType: TransactionFlagType; tooltip: string }): void => {
			it(`should set the tooltip when the flag type is ${String(
				scenario.flagType,
			)}`, (): void => {
				setup(scenario);
	...
	describe("element", (): void => {
		...
		it("should display the icon for the flag", (): Chai.Assertion =>
			expect(i.hasClass("glyphicon-flag")).to.be.true);

		it("should be transparent when the transaction is not flagged", (): Chai.Assertion =>
		...
		it("should be opaque when the transaction is flagged", (): void => {
			...
			transactionFlag.compile({ "transaction-flag": "model" });
			transactionFlag.scope.$digest();
			expect(transactionFlag["element"].find("i").hasClass("active")).to.be
				.true;
			expect(transactionFlag["element"].find("i").hasClass("noreceipt")).to.be
				.true;
		});
	});
