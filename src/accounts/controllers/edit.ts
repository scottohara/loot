...
export default class AccountEditController {
	...
	public accountTypes(filter?: string): DisplayAccountType[] {
		const types: DisplayAccountType[] = [
			"Asset",
			"Bank",
			"Cash",
			"Credit",
	...
	public accounts(filter: string, limit: number): angular.IPromise<Account[]> {
		return this.accountModel
			.all()
			.then((accounts: Account[]): Account[] =>
				this.limitToFilter(
					this.filterFilter(accounts, { name: filter, account_type: "asset" }),
					limit,
				),
			);
	}
	...
	public save(): void {
		...
		this.account.account_type =
			this.account.account_type.toLowerCase() as StoredAccountType;
		this.account.status =
			this.account.status.toLowerCase() as StoredAccountStatus;
