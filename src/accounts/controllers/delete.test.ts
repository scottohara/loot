...
describe("AccountDeleteController", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				...
				account = _account_;
				accountDeleteController = controllerTest(
					"AccountDeleteController",
				) as AccountDeleteController;
			},
		) as Mocha.HookFunction,
	);
	...
	it("should capitalise the account type", (): Chai.Assertion =>
		expect(accountDeleteController.account.account_type).to.equal("Bank"));

	it("should capitalise the status", (): Chai.Assertion =>
		expect(accountDeleteController.account.status).to.equal("Open"));
	...
	describe("deleteAccount", (): void => {
		...
		it("should display an error message when the account delete is unsuccessful", (): void => {
			...
			accountDeleteController.deleteAccount();
			expect(accountDeleteController.errorMessage as string).to.equal(
				"unsuccessful",
			);
		});
	});
