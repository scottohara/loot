...
describe("AccountReconcileController", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				...
				$window = _$window_;
				account = _account_;
				accountReconcileController = controllerTest(
					"AccountReconcileController",
				) as AccountReconcileController;
			},
		) as Mocha.HookFunction,
	);
	...
	it("should expect a postive closing balance to be entered by the user", (): Chai.Assertion =>
		expect(accountReconcileController.expectNegativeBalance).to.be.false);
