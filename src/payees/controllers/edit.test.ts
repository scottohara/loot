...
describe("PayeeEditController", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				...
				payee = _payee_;
				payeeEditController = controllerTest(
					"PayeeEditController",
				) as PayeeEditController;
			},
		) as Mocha.HookFunction,
	);
	...
	describe("when a payee is provided", (): void => {
		it("should make the passed payee available to the view", (): Chai.Assertion =>
			expect(payeeEditController.payee).to.deep.equal(payee));

		it("should set the mode to Edit", (): Chai.Assertion =>
			expect(payeeEditController.mode).to.equal("Edit"));
	});
	...
	describe("when a payee is not provided", (): void => {
		...
		it("should set the mode to Add", (): Chai.Assertion =>
			expect(payeeEditController.mode).to.equal("Add"));
	});
	...
	describe("save", (): void => {
		...
		it("should display an error message when the payee save is unsuccessful", (): void => {
			...
			payeeEditController.save();
			expect(payeeEditController.errorMessage as string).to.equal(
				"unsuccessful",
			);
		});
	});
