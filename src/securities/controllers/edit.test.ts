...
describe("SecurityEditController", (): void => {
	...
	beforeEach(
		angular.mock.inject(
			(
				...
				security = _security_;
				securityEditController = controllerTest(
					"SecurityEditController",
				) as SecurityEditController;
			},
		) as Mocha.HookFunction,
	);
	...
	describe("when a security is provided", (): void => {
		it("should make the passed security available to the view", (): Chai.Assertion =>
			expect(securityEditController.security).to.deep.equal(security));

		it("should set the mode to Edit", (): Chai.Assertion =>
			expect(securityEditController.mode).to.equal("Edit"));
	});
	...
	describe("when a security is not provided", (): void => {
		...
		it("should set the mode to Add", (): Chai.Assertion =>
			expect(securityEditController.mode).to.equal("Add"));
	});
	...
	describe("save", (): void => {
		...
		it("should display an error message when the security save is unsuccessful", (): void => {
			...
			securityEditController.save();
			expect(securityEditController.errorMessage as string).to.equal(
				"unsuccessful",
			);
		});
	});
