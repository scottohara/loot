...
describe("Schedule Edit", (): void => {
	let expected: ScheduleEdit, originalRowCount: number, lastSchedule: Schedule;
	...
	function commonBehaviour(): void {
		it("should not save changes when the cancel button is clicked", (): void => {
			...
			cy.get(schedulesTableRows)
				.last()
				.within((): void => checkRowMatches(lastSchedule));
		});
	...
	beforeEach((): void => {
		cy.login();
		cy.visit("/#!/schedules");
		cy.get(schedulesTableRows).then(
		...
		const tomorrow: Date = addDays(startOfDay(new Date()), 1),
			...
			schedules: ScheduleEdit[] = [
				...
				},
				{
					...
					categoryName: "Buy",
					subcategoryOrAccountName: "bank account 3",
					memo: "2.0000 @ $1.000 (plus $1.00 commission)",
					frequency: "Monthly",
					creditAmount: "~$3.00",
					rawNextDueDate,
				...
				},
				{
					...
					categoryName: "Sell",
					subcategoryOrAccountName: "bank account 3",
					memo: "2.0000 @ $1.000 (less $1.00 commission)",
					frequency: "Monthly",
					debitAmount: "~$1.00",
					rawNextDueDate,
					securityName: "Security 1",
		...
		schedules.forEach((schedule: ScheduleEdit): void => {
			describe(schedule.type ?? schedule.memo, (): void => {
				...
				it("should insert a new schedule when the save button is clicked", (): void => {
					...
					cy.get(schedulesTableRows).should(
						"have.length",
						originalRowCount + 1,
					);
