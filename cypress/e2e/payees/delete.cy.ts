...
describe("Payee Delete", (): void => {
	let originalRowCount: number, lastPayee: Payee, secondLastPayee: Payee;
	...
	describe("deleting a payee", (): void => {
		beforeEach((): void => {
			cy.get(payeesTableRows)
				.eq(originalRowCount - 1)
				.click();
			cy.get("body").type("{del}");
			cy.get(payeeDeleteHeading).should("have.text", "Delete Payee?");
		});

		it("should display the details of the payee being deleted", (): Cypress.Chainable<JQuery> =>
			cy.get(payeeToDelete).should("have.text", lastPayee.name));
		...
		it("should delete an existing payee when the delete button is clicked", (): void => {
			...
			cy.get(payeesTableRows)
				.last()
				.within((): void => checkRowMatches(secondLastPayee));
		});
