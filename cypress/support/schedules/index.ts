...
export function getValuesFrom(row: JQuery<HTMLTableRowElement>): Schedule {
	...
	return {
		...
		subcategoryOrAccountName: subcategoryOrAccount.length
			? subcategoryOrAccount.text().trim()
			: undefined,
		subtransactions: row.find(scheduleSubtransactionsToggleButton).length
			? []
			: undefined,
...
export function getSubtransactionRowValues(
	...
	return {
		...
		subcategoryOrAccountName: row
			.find(scheduleSubtransactionSubcategoryOrAccountName)
			.text()
			.trim(),
...
export function checkRowMatches(expectedValues: Schedule | ScheduleEdit): void {
	...
	cy.get(scheduleIsAutoEntered).should(
		isAutoEntered ? "be.visible" : "not.exist",
	);
	cy.get(schedulePrimaryAccountName).should("have.text", primaryAccountName);
	cy.get(schedulePayeeOrSecurityName).should(
		"contain.text",
		payeeOrSecurityName,
	);
	cy.get(scheduleCategoryName).should("contain.text", categoryName);

	if (undefined === subtransactions && undefined !== subcategoryOrAccountName) {
		cy.get(scheduleSubcategoryOrAccountName).should(
			"contain.text",
			subcategoryOrAccountName,
		);
	}

	cy.get(scheduleSubtransactionsToggleButton).should(
...
export function checkSubtransactionRowValues(
	...
	cy.get(scheduleSubtransactionCategoryName).should(
		"contain.text",
		categoryName,
	);
	cy.get(scheduleSubtransactionSubcategoryOrAccountName).should(
		"contain.text",
		subcategoryOrAccountName,
	);
	cy.get(scheduleSubtransactionMemo).should("have.text", memo);
	cy.get(scheduleSubtransactionAmount).should("contain.text", amount);
}
