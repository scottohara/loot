...
const transactionTransactionDate = "> td:first",
	...
	transactionCommissionLabel =
		"td:nth-of-type(3) > table > tbody > tr:last > td",
...
export function getValuesFrom(row: JQuery<HTMLTableRowElement>): Transaction {
	...
	return {
		...
		subcategoryOrAccountName: subcategoryOrAccountName.length
			? subcategoryOrAccountName.text().trim()
			: undefined,
		subtransactions: row.find(transactionSubtransactionsToggleButton).length
			? []
			: undefined,
		...
		balanceOrAmount: balanceOrAmount.length
			? balanceOrAmount.text().trim()
			: undefined,
	};
}

export function getSubtransactionRowValues(
	row: JQuery<HTMLTableRowElement>,
): Subtransaction {
	return {
		categoryName: row.find(transactionSubtransactionCategoryName).text().trim(),
		subcategoryOrAccountName: row
			.find(transactionSubtransactionSubcategoryOrAccountName)
			.text()
			.trim(),
...
export function checkRowMatches(
	...
	cy.get(transactionPayeeOrSecurityName).should(
		"contain.text",
		payeeOrSecurityName,
	);
	...
	if (undefined === subtransactions && undefined !== subcategoryOrAccountName) {
		cy.get(transactionSubcategoryOrAccountName).should(
			"contain.text",
			subcategoryOrAccountName,
		);
	}

	cy.get(transactionSubtransactionsToggleButton).should(
		undefined === subtransactions ? "not.exist" : "be.visible",
	);
...
export function checkSubtransactionRowValues(
	expectedValues: Subtransaction,
): void {
	const { categoryName, subcategoryOrAccountName, memo, amount } =
		expectedValues;

	cy.get(transactionSubtransactionCategoryName).should(
		"contain.text",
		categoryName,
	);
	cy.get(transactionSubtransactionSubcategoryOrAccountName).should(
		"contain.text",
		subcategoryOrAccountName,
	);
	cy.get(transactionSubtransactionMemo).should("have.text", memo);
	cy.get(transactionSubtransactionAmount).should("contain.text", amount);
}
