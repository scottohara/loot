...
export function checkRowMatches(expectedValues: Security, noTransactions = false): void {
	if (noTransactions) {
		cy.get(securityName).contains(new RegExp(`${expectedValues.name}\\s+No transactions`, "u")).should("exist");
	} else {
