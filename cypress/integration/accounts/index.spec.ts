...
describe("Account Index", (): void => {
	...
	before((): void => {
			...
			accountTypes: [
				{
					...
					accounts: [
						...
						{ name: "bank account 2", closingBalance: "$1,000.00", favourite: true }
					],
					total: "$1,500.00"
				},
				{
		...
		allAccounts = expected.accountTypes.map((type: AccountType): Account[] => type.accounts).flat();
		cy.createAccounts();
	});
	...
	it("should display a table for each account type", (): void => {
		// Number of tables
		cy.get(accountTypeTables).should("have.length", expected.accountTypes.length);

		cy.get(accountTypeTables).each((table: HTMLTableElement, index: number): void => {
			// Account Type
			cy.wrap(table).get(accountType).should("contain.text", expected.accountTypes[index].heading);
		});
	});
	...
	it("should display a row for each account", (): void => {
		...
		cy.get(accountsTableRows).each((row: HTMLTableRowElement, index: number): void => {
	...
	it("should display a related cash account link for investment accounts", (): void => {
		cy.get(investmentAccountsTableRows).each((row: HTMLTableRowElement): void => {
	...
	it("should display a total for each account type table", (): void => {
		cy.get(accountsTableTotals).should("have.length", expected.accountTypes.length);
		cy.get(accountsTableTotals).each((total: HTMLTableHeaderCellElement, index: number): void => {
			cy.wrap(total).should("have.text", expected.accountTypes[index].total);
		});
	});

	it("should display a total for all account types", (): Cypress.Chainable<JQuery> => cy.get(accountsTotal).should("have.text", expected.total));

	it("should indicate any accounts that are closed", (): Cypress.Chainable<JQuery> => cy.get(closedAccountTableRows).should("have.length", 1));
	...
	describe("table actions", (): void => {
		...
		describe("insert", (): void => {
			...
			it("should display the Add Account view when the insert key is pressed", (): Cypress.Chainable<JQuery<HTMLBodyElement>> => cy.get("body").type("{insert}"));
			it("should display the Add Account view when the CTRL+N keys are pressed", (): Cypress.Chainable<JQuery<HTMLBodyElement>> => cy.get("body").type("{ctrl}n"));
		});
		...
		describe("edit", (): void => {
			...
			it("should display the Edit Account view when the edit icon is clicked", (): Cypress.Chainable<JQuery> => cy.get(accountsTableRows).first().within((): void => {
		...
		describe("delete", (): void => {
			...
			it("should display the Delete Account view when the delete icon is clicked", (): Cypress.Chainable<JQuery> => cy.get(accountsTableRows).first().within((): void => {
		...
		describe("select", (): void => {
			...
			it("should display the Account Transactions view when the account name is clicked", (): void => {
				...
				cy.get(accountsTableRows).first().within((): void => {
			...
			it("should display the linked Account Transactions view when the linked cash account is clicked", (): void => {
				...
				cy.get(investmentAccountsTableRows).first().within((): void => {
