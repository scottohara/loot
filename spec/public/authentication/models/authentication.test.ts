...
describe("authenticationModel", (): void => {
	...
	describe("SESSION_STORAGE_KEY", (): void => {
		it("should be 'lootAuthenticationKey'", (): Chai.Assertion => expect(authenticationModel["SESSION_STORAGE_KEY"]).to.equal("lootAuthenticationKey"));
	});
	...
	describe("isAuthenticated", (): void => {
		...
		it("should fetch the authentication key from sessionStorage", (): void => {
			...
			expect($window.sessionStorage.getItem).to.have.been.calledWith("lootAuthenticationKey");
		});
		...
		describe("when authenticated", (): void => {
			...
			it("should set the default $http Authorization header", (): void => {
				$http.defaults = {};
				({ isAuthenticated } = authenticationModel);
				expect(($http.defaults.headers as angular.IHttpRequestConfigHeaders).common.Authorization).to.equal("Basic authentication key");
			});

			it("should update the default $http Authorization header", (): void => {
				$http.defaults.headers = { common: { Authorization: "" } };
				({ isAuthenticated } = authenticationModel);
				expect($http.defaults.headers.common.Authorization).to.equal("Basic authentication key");
			});
		...
		describe("when not authenticated", (): void => {
			it("should be false", (): Chai.Assertion => expect(authenticationModel.isAuthenticated).to.be.false);
		});
	});
	...
	describe("login", (): void => {
		...
		it("should save the authentication key to sessionStorage", (): Chai.Assertion => expect($window.sessionStorage.setItem).to.have.been.calledWith("lootAuthenticationKey", "base64 encoded"));

		it("should set the default $http Authorization header", (): Chai.Assertion => expect(($http.defaults.headers as angular.IHttpRequestConfigHeaders).common.Authorization).to.equal("Basic base64 encoded"));
	});

	describe("logout", (): void => {
		beforeEach((): void => authenticationModel.logout());

		it("should remove the authentication key from sessionStorage", (): Chai.Assertion => expect($window.sessionStorage.removeItem).to.have.been.calledWith("lootAuthenticationKey"));

		it("should clear the default $http Authorization header", (): Chai.Assertion => expect(($http.defaults.headers as angular.IHttpRequestConfigHeaders).common.Authorization).to.equal("Basic "));

		it("should clear all $http caches except the template cache", (): void => {
			expect($cache["removeAll"]).to.have.been.called;
			expect(($cacheFactory.get as SinonStub)("templates").removeAll).to.not.have.been.called;
		});
	});
});
